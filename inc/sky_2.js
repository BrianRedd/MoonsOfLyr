$(document).ready(function() {
    ...
    * Version 2.0
    ...
    var HoursPerDay = 20;
    var MoonData = [
        //name|size|orbit|incl|scion   |color",
        "Gold | 35 |10.35|  17|Body    |gold",
        "Mist | 40 |17.55|  11|Mind    |blue",
        "Fire | 45 |27.90|   5|Emotions|red"
    ];
    var NextConv = 13013676;

    /*Check query string parameters to modify above values*/
    ...
    if ($.urlParam("year") > 0) {
        StartYear = $.urlParam("year");
    };
    if ($.urlParam("day") > 0 && $.urlParam("day") < DaysPerYear) {
        StartDay = $.urlParam("day");
    };
    if ($.urlParam("hour") > 0 && $.urlParam("hour") < HoursPerDay) {
        StartHour = $.urlParam("hour");
    };
    if ($.urlParam("date") != 0) {
        var temp = $.urlParam("date").split("-");
        console.log("Date: " + temp[0] + "." + temp[1]);
        if (temp[1] > (DaysPerYear / 4)) temp[1] = 0;
        switch (temp[0].substr(0, 3).toLowerCase()) {
            case "aut":
                StartDay = parseInt(temp[1]);
                break;
            case "win":
                StartDay = parseInt(temp[1]) + (DaysPerYear / 4);
                break;
            case "spr":
                StartDay = parseInt(temp[1]) + (DaysPerYear / 2);
                break;
            case "sum":
                StartDay = parseInt(temp[1]) + (3 * DaysPerYear / 4);
                break;
        }
    };
    ...
    var seasonN = ["Autumn", "Winter", "Spring", "Summer"];
    ...
    function setMoonObj(name, size, period, inc, scion, color) {
    ...
    function setMoon(x) {
        ...
        Moon[x] = new setMoonObj(temp[0], temp[1], temp[2], temp[3], temp[4], temp[5]);
        //console.log("Moon Object Data: name: " + Moon[x].name + ", angle: " + Moon[x].angle);
        ...
        switch (key) {
            ...
            case 82: //keyboard "r"
                ...
                /*case 18: //keyboard "t"
                	...
                	break;*/
            case 191: //keyboard "?"
                alert("Query strings: year, day, hour, date [season-day]");
                break;
        }
    });
    ...
    function snapToMoon(x) {
        //console.log("snapToMoon " + x + ", name: " + Moon[x]);
    ...
    function MoonInfo(x) {
        ...
        console.log("Period: " + Moon[x].period);
        console.log("Darklevel: " + Moon[x].darklevel);
        console.log("Inclination: " + Moon[x].inclination);
        ...
        console.log("Shadow Angle: " + Moon[x].angle);
        MNI$.fadeIn().css({
            "top": Math.round(Moon[x].y + Moon[x].base - 35 + (Moon[x].size / 2)) + "px",
            "left": Math.round(Moon[x].x + Moon[x].base - 65 + (Moon[x].size / 2)) + "px"
        });
        MNI$.find("#MIname").text(Moon[x].name.toUpperCase() + "MOON").css("color", Moon[x].color);
        MNI$.find("#MIscion").text("Scion of " + Moon[x].scion);
        MNI$.find("#MIperiod").text(Math.round(Moon[x].period) + " days");
    };
    ...
    function SunInfo() {
        ...
        console.log("Planet Angle: " + SkyAngle);
    };
